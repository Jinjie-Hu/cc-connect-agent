...
| [整机综合对比准备计划.md](./整机综合对比准备计划.md) | 整机（`detector`，528 G-阵列 MAC PE + 32 MF_PE）BEFORE/AFTER 综合对比的**准备计划** | 2026-09-07 | 定义 BEFORE（git HEAD compact）vs AFTER（fused 工作树）两版与 filelist 差异；选定流程方案 A（logic、no retime，与 PE 级同口径）；列出 RAM 映射等整机特有核查项与前置条件（先 0-LSB FC 通过再综合）；**AFTER 算术引用已统一指向 `arithmetic_unit_full/`（字节级相同、bit-safe）并整机 FC 0/32 复验**；整机综合仍未跑 |
...
5. **2026-09-06（后段）** 其余算术单元统一 full-lib：核对 LPJG_unit / A_systolic_array / detector 其余算术单元（`complex_adder`/`complex_addsub`/`complex_real_multiplier`/`real_multiplier`/`real_adder`/`real_addsub`/`complex_conj`/`complex_negative`/`real_negative`/`reciprocal_lut.v`）compact↔full **逐字节相同**（本地行级 diff + EDA md5 双验证）；full 与 compact 唯一内容差异 = `complex_multiplier` + fused 新增模块 → 整机 AFTER 的 `sim_rtl_fc.f` / `syn_whole/rtl_after.filelist` 算术引用**统一指向 `arithmetic_unit_full/`**（文件级零语义改动、bit-safe），EDA 重跑整机 fused FC → **PASS 0/32**（`vcs_top_fc_16_optb_full`，golden `29cdbe71`）。对整机面积对比**无新增差异**——BEFORE/AFTER 面积差仍只来自 560 PE 的 fused MAC（Option B 加宽）+ LPJG_unit `complex_multiplier` 单次截位。
