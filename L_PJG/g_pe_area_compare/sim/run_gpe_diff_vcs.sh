#!/bin/bash
# Differential VCS functional check: compact G_PE vs fused G_PE_fused (hoisted complex_mac_vcs)
...
echo "===== [2/2] compile + run fused G_PE_fused (complex_mac_vcs) ====="
vcs -sverilog +v2k -timescale=1ns/1ns -top tb_gpe_diff -full64 -j8 \
    -o sim_pe_f -l compile_f.log +define+USE_FUSED \
    -f ../syn/script/g_pe_fused/G_PE_fused_vcs.filelist tb_gpe_diff.sv > build_f.txt 2>&1
rc=$?
