...
module complex_adder_tree_tb;

	localparam qtz_t DUT_I_QTZ  = get_qtz(1, 9, 6, COMPLEX);
	...
	initial begin
		...
		fd = $fopen("data/complex_adder_tree_input.txt", "r");
		if (fd == 0) $fatal(1, "[FATAL] Cannot open data/complex_adder_tree_input.txt");
		...
		fd = $fopen("data/out/complex_adder_tree_expected.txt", "r");
		if (fd == 0) $fatal(1, "[FATAL] Cannot open complex_adder_tree_expected.txt");
		scan_ok = $fscanf(fd, "%x\n", expected);
		$fclose(fd);
