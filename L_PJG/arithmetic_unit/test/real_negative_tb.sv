...
module real_negative_tb;

	localparam qtz_t DUT_QTZ = get_qtz(1, 9, 6, REAL);

	reg  signed [DUT_QTZ.width-1:0] stim_a;
	wire signed [DUT_QTZ.width-1:0] dut_out;
	reg  signed [DUT_QTZ.width-1:0] expected;
	...
	initial begin
		...
		fd = $fopen("data/out/real_negative_expected.txt", "r");
		if (fd == 0) $fatal(1, "[FATAL] Cannot open real_negative_expected.txt");
		scan_ok = $fscanf(fd, "%x\n", expected);
		$fclose(fd);
