...
module complex_mac_w_bias_tb;
	...
	initial begin
		...
		fd = $fopen("data/complex_mac_w_bias_input.txt", "r");
		if (fd == 0) $fatal(1, "[FATAL] Cannot open data/complex_mac_w_bias_input.txt");
		...
		fd = $fopen("data/out/complex_mac_w_bias_expected.txt", "r");
		if (fd == 0) $fatal(1, "[FATAL] Cannot open complex_mac_w_bias_expected.txt");
		scan_ok = $fscanf(fd, "%x\n", expected);
		$fclose(fd);
