...
module real_adder_tb;
	...
	localparam DUT_SUB_EN      = 0;  // 0 = add, 1 = sub
	...
	initial begin
		...
		fd = $fopen("data/out/real_adder_expected.txt", "r");
		if (fd == 0) $fatal(1, "[FATAL] Cannot open real_adder_expected.txt");
		scan_ok = $fscanf(fd, "%x\n", expected);
		$fclose(fd);
